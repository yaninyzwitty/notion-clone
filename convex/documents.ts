    ...
    handler: async (ctx, args) => {
        ...
        const recursiveArchive = async (documentId: Id<"documents">) => {
            ...
            for (const child of children) {
                ...
                await recursiveArchive(child._id); //check they do not have any children
            }
        }
...
export const getTrash = query({
    async handler(ctx) {
        ...
        const userId = identity.subject;
        const documents = ctx.db.query("documents").withIndex("by_user", q => q.eq("userId", userId)).filter(q => q.eq(q.field("isArchived"), true)).order("desc").collect();
        return documents;
...
export const restore = mutation({
    ...
    async handler(ctx, args) {
        ...
        if (existingDocument.userId !== userId) {
          ...
          const recursiveRestore = async function (documentId:Id<"documents">)  {
